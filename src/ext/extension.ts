...
export function activate(context: vscode.ExtensionContext) {
  ...
  console.log(
    'Congratulations, your extension "vscode-webview-react-boilerplate" is now active!'
  );
  ...
  let disposable = vscode.commands.registerCommand(
    'vscode-webview-react-boilerplate.helloWorld',
    () => {
      // The code you place here will be executed every time your command is executed
      // Display a message box to the user
      vscode.window.showInformationMessage(
        'Hello World from vscode-webview-react-boilerplate!'
      );
    }
  );
  ...
  let sendTextToTerminalCommand = vscode.commands.registerCommand(
    'extension.openTerminal',
    (msg: string) => {
      if (vscode.env.uiKind === vscode.UIKind.Desktop) {
        // Find an existing terminal
        let terminal = vscode.window.terminals.find(
          (t) => t.name === 'Extension terminal'
        );
        if (!terminal) {
        ...
        vscode.window.showWarningMessage(
          'Terminal creation is not supported in this environment.'
        );
      }
    }
  );
